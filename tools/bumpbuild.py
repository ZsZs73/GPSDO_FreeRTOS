#!/usr/bin/env python3
"""bumpbuild.py — increment BUILD_SERIAL in build_id.h.
...
The sketch includes build_id.h, so touching that file is enough to make the
...
path = root / "build_id.h"

if not path.exists():
    sys.exit(f"bumpbuild: no build_id.h in {root}")

text = path.read_text(encoding="utf-8")
m = re.search(r"^(#define\s+BUILD_SERIAL\s+)(\d+)\s*$", text, re.M)
if not m:
    sys.exit("bumpbuild: no '#define BUILD_SERIAL <n>' line in build_id.h")
